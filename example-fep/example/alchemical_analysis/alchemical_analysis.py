#!/usr/bin/env python
#yhrun -N 1 -p cp1 python alchemical_analysis.py -d ../xvg/ -o . -m -dexp-iexp -r 6 -g -w
...
def uncorrelate(sta, fin, do_dhdl=False):
   ...
   if UNCORR_OBSERVABLE == 'dhdl':
      ...
      for k in range(K):
         ...
         indices = sta[k] + numpy.array(pymbar.timeseries.subsample_correlated_data(dhdl_sum, g=g[k])) # indices of uncorrelated samples
   ...
   if UNCORR_OBSERVABLE == 'dE':
      ...
      for k in range(K):
         ...
         g[k] = pymbar.timeseries.statistical_inefficiency(dE)
         indices = sta[k] + numpy.array(pymbar.timeseries.subsample_correlated_data(dE, g=g[k])) # indices of uncorrelated samples
         N = len(indices) # number of uncorrelated samples
         # Handle case where we end up with too few.
         if N < P.uncorr_threshold:
            print("WARNING: Only %s uncorrelated samples found at lambda number %s; proceeding with analysis using correlated samples..." % (N, k))
   ...
   for k in range(K-1):
      ...
      for name in P.methods:
         ...
         if name == 'BAR':
            ...
            #===================================================================================================
            result = pymbar.bar(w_F, w_R, relative_tolerance=P.relative_tolerance, verbose = P.verbose)
            df['BAR'] = result['Delta_f']
            ddf['BAR'] = result['dDelta_f']
            print(f"df['BAR']为：{df['BAR']}")
         if name == 'UBAR':
            #===================================================================================================
            # Estimate free energy difference with unoptimized BAR -- assume dF is zero, and just do one evaluation
            #===================================================================================================
            result = pymbar.bar(w_F, w_R, verbose = P.verbose,iterated_solution=False)
            df['UBAR'] = result['Delta_f']
            ddf['UBAR'] = result['dDelta_f']
         if name == 'RBAR':
            #===================================================================================================
            ...
            for trial_udf in range(-10,10,1):
               result = pymbar.bar(w_F, w_R, DeltaF=trial_udf, iterated_solution=False, verbose=P.verbose)
               udf = result['Delta_f']
               uddf = result['dDelta_f']
...
def totalEnergies():
   ...
   for i in range(len(segments)):
      ...
      for name in P.methods:
         ...
         # Use the total energy value and uncertainty that pymbar offers.
            ...
            for k in range(segstart,segend):
               print(f"dF[{name}] (before):", dF[name], type(dF[name]))
               print(f"df_allk[{k}][{name}]:", df_allk[k][name], type(df_allk[k][name]))
               print(df_allk[:5])
   ...
   # outfile = open(os.path.join(P.output_directory, '%s.pickle'%P.resultfilename), 'b')
