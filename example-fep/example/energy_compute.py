...
# command : python energy_compute.py complex.xvg HMX.xvg lig.xvg -f energy_results.xvg

# 用于计算HMX配体之间的相互作用，
# 通过 E = HMXlig - HMX - lig 求得能量
# 
# 输入一共需要三个文件：HMX配体的能量xvg文件，HMX的能量xvg文件，配体的能量xvg文件
...
# 结果文件 energy_results.xvg 可以用 xvgshow.py 可视化
#################################################
...
def energy_compute():
    try:
        ...
        if os.path.exists(HMX_lig_file):
            HMX_lig_title, HMX_lig_xlabel, HMX_lig_ylabel, HMX_lig_data = xvg_deal(HMX_lig_file)
        else:
            print("HMX_lig_file not exists in this directory ")
            return 
        if os.path.exists(HMX_file):
            HMX_title, HMX_xlabel, HMX_ylabel, HMX_data = xvg_deal(HMX_file)
        else:
            print("HMX_file not exists in this directory ")
            return 
        ...
        if len(HMX_lig_data) != 5 or len(HMX_data) != 5 or len(lig_data) != 5:
        ...
        final_data = HMX_lig_data
        for i in range(1, len( final_data)):
            for j in range(1, len(final_data[i])):
                final_data[i][j] -= HMX_data[i][j] + lig_data[i][j]
                # print(final_data[i][j], HMX_data[i][j], lig_data[i][j])
        ...
        with open("energy_results" + filename_output + ".xvg", 'w') as fo:
            ...
            fo.write(HMX_lig_file + ', ' + HMX_file + ' and ' + lig_file + '\n')
        ...
        HMX_lig_title += " generated from energy_compute.py "
        if plotmode == '-s':
            picture_subplot(HMX_lig_title, HMX_lig_xlabel, HMX_lig_ylabel, final_data, cols)
        elif plotmode == '-o':
            picture_oneplot(HMX_lig_title, HMX_lig_xlabel, HMX_lig_ylabel, final_data, cols)
